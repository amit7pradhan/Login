...
function Login() {
  ...
  const getPasswordErrors = (password) => {
    ...
    if (password.length === 0)
      return [
        "Password must contain at least 8 characters, one capital letter, one symbol (@$!%*?&), and one number",
      ];
  ...
  return (
    <div className="container">
      <img src={logo} alt="Logo" className="logo" />
        <h1>
        {/* ✅ Replaced <a> tags with <Link> for client-side routing */}
      ...
      <form onSubmit={isOTP ? handleOTP : handleLogin}>
        ...
        {isOTP && (
          <div className="resend-container">
            <button type="button" className="resend-btn" onClick={handleOTP}>
              Resend OTP
            </button>
          </div>
        )}
